...
export default function WelcomeSlidesPage() {
  ...
  // Gesture consent state (null = unanswered). persisted to localStorage.
  const [gestureConsent, setGestureConsent] = useState<boolean | null>(null)
  useEffect(() => {
    try {
      const v = localStorage.getItem('gestureConsent')
      if (v !== null) setGestureConsent(v === 'true')
    } catch {
      /* ignore */
    }
  }, [])

  const answerGestureConsent = useCallback((choice: boolean) => {
    setGestureConsent(choice)
    try {
      localStorage.setItem('gestureConsent', choice ? 'true' : 'false')
    } catch {
      /* ignore */
    }
  }, [])
        ...
        <section className="w-full flex-1 relative overflow-hidden">
          ...
          >
            {SLIDES.map((s) => (
              <article
                key={s.id}
                className="shrink-0 w-screen h-full flex flex-col justify-center items-center px-6 pointer-events-none"
              >
                {/* Welcome Slide - What is Swipe? */}
                {s.id === 0 && (
                  <div className="w-full max-w-4xl mx-auto pointer-events-auto px-4 sm:px-6">
                    <Card className="shadow-lg">
                      ...
                      <CardContent className="space-y-4 sm:space-y-6">
                        <p className="text-base sm:text-lg lg:text-xl leading-relaxed font-medium text-secondary-foreground">
                          Swipe lets you present by just using your hands,
                          making it fun and easy—almost like magic.
                        </p>
                        <p className="text-base sm:text-lg lg:text-xl leading-relaxed text-secondary-foreground/90">
                          Swipe is a tool that lets you control your
                          presentation using only your hands. Instead of
                          clicking a mouse or pressing a button, you can wave
                          your hand to go to the next slide, move back, zoom in,
                          or look around a slide.
                        </p>
                        <p className="text-base sm:text-lg lg:text-xl leading-relaxed text-secondary-foreground/90">
                          A camera watches your hand movements, and Swipe's
                          smart computer model figures out what action you want
                          to do. It also has a simple website where you can
                          upload your slides and present easily.
                        </p>
                      </CardContent>
                ...
                {/* PDF Upload Slide */}
                {s.id === 1 && (
                  <div className="mt-8 flex flex-col items-center gap-4 w-full max-w-2xl mx-auto pointer-events-auto">
                    {!pdfFile ? (
                      <label 
                        className="flex flex-col items-center justify-center w-full h-72 border-4 border-dashed rounded-lg cursor-pointer hover:border-blue-500"
                        onDragOver={(e) => e.preventDefault()} //new code
                        onDrop={(e)=> {
                          e.preventDefault();
                          const files = e.dataTransfer.files;
                          if (files && files[0]) {
                            setPdfFile(files[0]); // handle dropped file
                          }
                        }}
                      >
                        ...
                        <input
                          ...
                          //onDragOver={(e) =>{enableDropping} onDrop={enableDrop}//e.dataTransfer.setData('text/plain', 'item1')}
                        />
                      </label>
                    ) : (
                      <>
                ...
                {s.id === 2 && (
                  <div className=" flex flex-col items-center gap-4 pointer-events-auto">
                    <div className="w-150 h-100 bg-black rounded-lg overflow-hidden border shadow flex items-center justify-center">
                      <video
                        ref={videoRef}
                ...
                {s.id === 3 && (
                  ...
                  <button
                    className="mt-8 px-6 py-2 rounded-lg bg-primary text-white font-semibold cursor-pointer hover:bg-primary-hover transition pointer-events-auto"
          ...
          <button
            ...
            className="group absolute z-50 top-1/2 left-3 -translate-y-1/2 hidden sm:flex items-center justify-center 
                      ...
                      hover:bg-blue-500 transition-colors cursor-pointer pointer-events-auto"
          >
            <ChevronLeft className="w-5 h-5 text-secondary-foreground group-hover:text-white transition-colors" />
          </button>
          ...
          <button
            ...
            className={`group absolute z-50 top-1/2 right-3 -translate-y-1/2 hidden sm:flex items-center justify-center 
                      ...
                      ${
                        index === 1 && !pdfFile
                          ? 'bg-gray-200 cursor-not-allowed opacity-50 border-gray-300 pointer-events-none'
                          : 'bg-tertiary/50 border-tertiary/60 hover:bg-blue-500 hover:text-white cursor-pointer pointer-events-auto'
                      }`}
          >
                  ...
                  <button
                    ...
                    className={`rounded-full transition-all duration-300 ${
                      isBlocked
                        ...
                        : isActive
                        ? 'w-3 h-3 bg-gray-700 cursor-pointer'
          ...
          {/* Gesture consent banner (cookie-style) */}
          {index === 2 && gestureConsent === null && (
            <div
              className="fixed left-4 right-4 bottom-6 md:left-20 md:right-20 z-50 pointer-events-auto"
              style={{ maxWidth: '1200px', margin: '0 auto' }}
            >
              <div className="bg-background border rounded-lg shadow-lg p-4 flex flex-col md:flex-row items-center justify-between gap-4">
                ...
                <div className="flex gap-3">
                  <button
                    onClick={() => answerGestureConsent(true)}
                    className="px-4 py-2 bg-primary text-white rounded cursor-pointer hover:bg-primary-hover transition"
                  >
                    Yes
                  </button>
                  <button
                    onClick={() => answerGestureConsent(false)}
